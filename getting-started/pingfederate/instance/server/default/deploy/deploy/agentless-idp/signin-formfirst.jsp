...
This is a basic example of an Agentless IDP integration with PingFederate.  This one is
designed to run as a "form-first" adapter.  When the SSO session is initiated from the
adapter rather than through the regular PF process.
...
private final static String PF_ENTITY_ID = "example:saml20:sp"; // PartnerSpId value for the startSSO.ping action
...
try
{
	// 1 - Implement any authentication logic (ie gather the currently logged on user, display a form etc)
	...
	String PARAM_REFERENCE_VALUE = dropOffAttributesToRefererenceIdAdapter( returnAttributes );


	// 3 - Redirect to startSSO process in PF with the REF included
	response.sendRedirect(PF_BASE_URL + ":" + PF_PRIMARY_SSL_PORT + PF_START_SSO_ENDPOINT + "?PartnerSpId=" + PF_ENTITY_ID + "&REF=" + PARAM_REFERENCE_VALUE);
