...
This is a basic example of an Agentless IDP integration with PingFederate.  This provides
Single Log Out (SLO) for the IDP adapter.
...
try
{
	// 1 - Gather the REF value provided to the SLO page
	...
	if ( PARAM_REFERENCE_VALUE == null ) {
		// we didn't receive a REF param.  Can we assume that we are doing a back-channel
		// logout?  If so, we need to grab the attribute passed to the signout page and
		// kill the appropriate session.
		
		// For this example. We just end...
	} else {
		...
		// 3 - Implement the logout logic.. kill the session, kill the cookies, clear sessions from databases


		String PARAM_RESUME_PATH = returnAttributes.get("resumePath");
	
	
		// 4 - Redirect to PF to continue SSO
	
		response.sendRedirect(PF_BASE_URL + ":" + PF_PRIMARY_SSL_PORT + PARAM_RESUME_PATH + "?source=" + REFID_INSTANCE_ID);
	}
