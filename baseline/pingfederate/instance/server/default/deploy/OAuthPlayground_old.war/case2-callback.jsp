...
<%@include file="header.jsp" %>
    ...
    <p>Following the OAuth 2.0 specification the callback URL received the token details as parameters in the hash fragment.  The hash fragment is used to prevent sending the parameters back to the server, as the intent is they will be used client side (e.g.: by JavaScript).  View the source of this page for sample JavaScript on parsing these attributes.</p>
                    ...
                    if (pair[0] == "access_token")
                    {
						...
						document.write("<span class=\"side-by-side\"> \
											<form action='token-revocation-endpoint-proxy.jsp' method='post'> \
												<input type='hidden' name='client_id' value='<%=im_client_id%>'> \
												<input type='hidden' name='client_secret' value='<%=im_client_secret%>'> \
												<input type='hidden' name='token' value=" + pair[1] + "> \
