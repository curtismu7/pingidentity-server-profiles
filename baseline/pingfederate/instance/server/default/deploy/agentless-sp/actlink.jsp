...
This is a basic example of an Agentless SP integration with PingFederate.  This is an example
for an SP adapter to perform Account Linking.
...
try
{
	// 1 - Gather the REF value provided to the Account Link page
	...
	String PARAM_RESUME_PATH = returnAttributes.get("resumePath"); // we will use this later


	// 3 - Implement the account linking logic.. locally sign the user in to determine local account. 
	// You have resumePath and partnerEntityID to help you

	//For this example we will return the local account id of "A12345"
	Map<String, String> accountLinkAttributes = new HashMap<String, String>();
	accountLinkAttributes.put("subject", "A12345");


	// 4 - Return the local account identifier back to PingFederate to use for future SSO actions.
	String PARAM_REFERENCE_VALUE_FOR_RESUME = dropOffAttributesFromReferenceIdAdapter( accountLinkAttributes );
	
	
	// 5 - Redirect to PF to continue account link
	response.sendRedirect(PF_BASE_URL + ":" + PF_PRIMARY_SSL_PORT + PARAM_RESUME_PATH + "?REF=" + PARAM_REFERENCE_VALUE_FOR_RESUME);
