...
This is a basic example of an Agentless IDP integration with PingFederate.  This is designed to run as an adapter and 
be called from any SSO connection (ie Traditional Adapter).

This will return two sample values back to the PingFederate process:
	subject -> jsmith
	realm -> corp
...
private final static String PF_START_SSO_ENDPOINT = "/idp/startSSO.ping"; // for "form-first" idp-initiated SSO
...
private final static String REFID_USERNAME = "idpuser"; // Config: [USER NAME]
private final static String REFID_PASSWORD = "idppassword"; // Config [PASS PHRASE]
...
try
{
	// 1 - Gather the authentication options from the parameters
	// Depending on the [TRANSPORT MODE] config parameter you will receive these params via POST or GET
	
	String PARAM_RESUME_PATH = request.getParameter("resumePath");
	String PARAM_REAUTH = request.getParameter("reauth");
	String PARAM_ALLOW_INTERACTION = request.getParameter("allowInteraction");


	// 2 - Implement any authentication logic (ie gather the currently logged on user, display a form etc)
	
	Map<String, String> returnAttributes = new HashMap<String, String>();
	returnAttributes.put("subject", new String("jsmith"));
	returnAttributes.put("realm", new String("corp"));


	// 3 - Send the user attributes to the referenceID adapter and get the returned reference value
	
	String idpReferenceValue = dropOffAttributesToRefererenceIdAdapter( returnAttributes );


	// 4 - Redirect to PF to continue SSO
	
	response.sendRedirect(PF_BASE_URL + ":" + PF_PRIMARY_SSL_PORT + PARAM_RESUME_PATH + "?REF=" + idpReferenceValue);
